...
* **手速過快與聲調提前自動校正**  
  打字過快時兩手手指同時落下，容易把聲調鍵按在韻母前面（例如打「只有紅茶可以嗎」時將「有」誤按成 `u3.` 或「紅」誤按成 `cj6/`）。模組內建順序自動重排機制，能依大千式注音鍵盤的鍵位角色將提前落下的聲調移回字尾，同時支援零聲母字與連打情境，並具備防誤觸檢查，確保只有在音節缺調時才觸發移動，不影響相鄰正常字詞。
