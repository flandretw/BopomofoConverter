...
* 🎯 **標點符號與多詞不破碎 (Delimiter Segmentation)**
  以往打字時若緊貼全形標點（如 `，`、`。`、`！`、`？`、`（`、`）`）或英文標點，容易導致音節混亂或翻譯斷裂破碎。全新邊界分離演算法能智慧識別非注音符號，即使整句混雜中文、全形標點、括號與多組注音（例如 `貓咪大戰爭，好好玩（al au 2845045/ ，cl3cl3j06）`），每個注音片段依然能夠完整獨立識別與翻譯，標點原樣保留不破碎！
* ⚡ **手速過快 / 聲調提前自動校正**
  ...
  - **結構角色自動重排**：大千式注音鍵盤的按鍵角色（聲母 Initial、介音 Medial、韻母 Final、聲調 Tone）各自獨立。當系統偵測到「聲母+聲調+介音/韻母」的顛倒組合（例如 `s3u` $\rightarrow$ `su3`、`148` $\rightarrow$ `184`、`2u3l` $\rightarrow$ `2ul3`），會在解析前自動把提早落下的聲調移回字尾歸位！
  - **零聲母字完美支援**：即使遇到沒有聲母的字（例如「一」`6u` $\rightarrow$ `u6`、「愛」`49` $\rightarrow$ `94`），甚至連字盲打時（如 `5k4g46uek7` $\rightarrow$ `5k4g4u6ek7`），也能智慧識別並修正！
...
本專案採用 **Stonecutter** 來進行跨版本（1.20.1、1.21.11、26.2）的開發與編譯管理，無需手動切換分支或複製程式碼。您可以透過以下 Gradle 指令來進行開發與測試：

* **編譯所有支援的版本：**
  ```bash
  .\gradlew clean buildAndCollect
  ```
  *(編譯完成的各版本 `.jar` 檔案會集中放置於專案根目錄的 `build/libs/` 下)*

* **啟動特定版本的測試客戶端：**
  我們已經將各個版本的測試環境完全隔離 (擁有各自的 `run` 目錄)，您可以直接透過指令啟動特定版本測試，例如：
  - 測試 1.20.1 版本：`.\gradlew :1.20.1:runClient`
  - 測試 1.21.11 版本：`.\gradlew :1.21.11:runClient`
  - 測試 26.2 版本：`.\gradlew :26.2.x:runClient`

* **切換 IDE 活躍編輯版本 (Active Stonecutter Project)：**
  ```bash
